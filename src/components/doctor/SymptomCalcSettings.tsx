...
export function SymptomCalcSettings({
  ...
  const [weight, setWeight] = useState(setting?.custom_weight ?? symptom.default_weight);
  ...
  const weightOverridden = setting?.custom_weight != null;
  const methodOverridden = setting?.calculation_method != null;

  async function save(update: { weight?: number | null; calculationMethod?: CalculationMethod | null }) {
    setSaving(true);
    setSavedNote(null);
  ...
  return (
    <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1.5 rounded-lg bg-zinc-50 px-3 py-2 text-xs text-zinc-600">
      ...
      {(weightOverridden || methodOverridden) && (
        <button
          type="button"
          disabled={saving}
          onClick={() => {
            setWeight(symptom.default_weight);
            setMethod(symptom.default_calculation_method);
            save({ weight: null, calculationMethod: null });
          }}
